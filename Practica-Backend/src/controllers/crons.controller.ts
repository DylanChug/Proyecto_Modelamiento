...
const validarSecretKey = (req: Request): boolean => {
...
function calcularDiferenciaHoras(inicio: string, fin: string): number {
...
export const ejecutarCronDiario = async (req: Request, res: Response) => {
    ...
    (async () => {
        ...
        try {
            ...
            const promesasReportes = usuariosConActividades.map((usuario) => {
                if (usuario.actividades.length === 0) return Promise.resolve();
                ...
                .then(response => {
                    if (response.error) console.error(`❌ Error de Resend en reporte matutino para ${usuario.id_usuario}:`, response.error.message);
                })
            ...
            if (actividadesAyer.length > 0) {
                ...
                const analisisUsuarios: Record<string, { horasEstudio: number, horasLibre: number, completadas: number }> = {};
...
export const ejecutarCronAlertas = async (req: Request, res: Response) => {
    if (!validarSecretKey(req)) {
        return res.status(401).json({ status: 'ERROR', message: 'No autorizado.' });
    }
