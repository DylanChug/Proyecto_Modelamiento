...
const sanitizarDirectrices = (texto) => {
    if (!texto)
        return "";
...
export const generarPropuestaOptimizada = async (req, res) => {
    try {
...
export const reoptimizarAgendaSemanal = async (req, res) => {
    try {
        const idUsuario = req.usuario?.id_usuario || req.usuario?.id || req.usuario?.email;
        if (!idUsuario) {
            return res.status(401).json({ status: 'ERROR', message: 'Sesión inválida.' });
        ...
        const actividadesParaFront = nuevaAgendaPropuesta.map((act) => {
...
export const confirmarReoptimizacionMasiva = async (req, res) => {
    try {
        const idUsuario = req.usuario?.id_usuario || req.usuario?.id || req.usuario?.email;
        if (!idUsuario) {
            return res.status(401).json({ status: 'ERROR', message: 'Sesión inválida.' });
